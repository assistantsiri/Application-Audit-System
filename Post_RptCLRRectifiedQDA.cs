...
namespace DA
{
   public class Post_RptCLRRectifiedQDA
    {
       string con = ConfigurationManager.ConnectionStrings["AASConnectionString"].ToString();
       public DataTable FetchCLRRectifiedQ_Post(Post_RptCLRRectifiedQBO objbo)
       {
           OracleConnection conn = new OracleConnection(con);
           conn.Open();
           OracleDataAdapter dAd = new OracleDataAdapter("RPTCLRRECTIFIEDQUERIES_Post", conn);
       ...
       public DataTable FetchApplicationName_Post(Post_RptCLRRectifiedQBO objbo)
       {
           OracleConnection conn = new OracleConnection(con);
           conn.Open();
